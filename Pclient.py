...
def sendMsg(aes_key):                          #Nothing sending messages to the server here....
    while True:
        
        client_msg = input("Enter message: ")                      #Message to send
        if client_msg.lower() == "exit":
            print("Bye!")
            os._exit(0)   # hard exit of program, Not sure if this is a 'nice' way to accomplish this
...
def start_client(host, port):
    ...
    client_thread = threading.Thread(target=sendMsg, daemon=True, args=([aes_key]))   #[aes_key] i believe this is correct
